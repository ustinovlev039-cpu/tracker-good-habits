...
class Command(BaseCommand):
    ...
    def process_update(self, base_url, update):
        ...
        if len(parts) != 2:
            self.send(base_url, chat["id"], "Откройте ссылку привязки из приложения или отправьте /start КОД.")
            return
        ...
        try:
            ...
            self.send(base_url, chat["id"], "Код привязки не найден или устарел.")
            return

        existing = TelegramProfile.objects.filter(chat_id=chat["id"]).exclude(pk=profile.pk).first()
        if existing:
            existing.chat_id = None
            existing.username = ""
            existing.is_verified = False
            existing.save(update_fields=["chat_id", "username", "is_verified", "updated_at"])

        profile.chat_id = chat["id"]
        profile.username = chat.get("username", "")
        profile.is_verified = True
        profile.save(update_fields=["chat_id", "username", "is_verified", "updated_at"])
        self.send(base_url, chat["id"], "Telegram успешно подключён. Напоминания будут приходить в этот чат.")

    def send(self, base_url, chat_id, text):
        requests.post(f"{base_url}/sendMessage", json={"chat_id": chat_id, "text": text}, timeout=15).raise_for_status()
